...
const mockContracts: Contract[] = [
  { id: 1, title: "Manutenção de Ficus", price: "R$ 150", client: "João Silva", status: "Finalizado" },
  { id: 2, title: "Projeto Horta Urbana", price: "R$ 450", client: "Maria Oliveira", status: "Em andamento" },
  { id: 3, title: "Poda de Bonsai", price: "R$ 120", client: "Carlos Costa", status: "Finalizado" },
  { id: 4, title: "Consultoria Paisagismo", price: "R$ 300", client: "Ana Santos", status: "Cancelado" },
  { id: 5, title: "Recuperação de Orquídeas", price: "R$ 90", client: "Pedro Alves", status: "Finalizado" },
  { id: 6, title: "Cuidados Mensais", price: "R$ 200", client: "Julia Mendes", status: "Em andamento" },
];
  ...
  return (
    <div className="rounded border border-black/10 bg-white p-3 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex h-44 items-center justify-center bg-gray-100 text-gray-400 rounded-sm">
        <svg viewBox="0 0 24 24" className="h-16 w-16" fill="none" stroke="currentColor" strokeWidth="1.4">
      ...
      <div className="mt-3 text-sm text-gray-800">
        ...
        <p className="mt-1 text-xs text-gray-500">
          Cliente: <span className="font-medium text-gray-700">{contract.client}</span>
        </p>

        <div className="mt-3 flex items-center justify-between">
          <strong className="font-black text-[#364153] text-lg">{contract.price}</strong>
          <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${
            contract.status === "Finalizado" ? "bg-green-100 text-green-700" : 
            contract.status === "Em andamento" ? "bg-blue-100 text-blue-700" : 
...
export function ProviderHistoryPage() {
  ...
  useEffect(() => {
    async function carregarContratosPrestador() {
      try {
        setIsLoading(true);
        const resposta = await api.get('/prestadores/contratos');
  ...
  const contratosFiltrados = contratos
    .filter(contract => 
      ...
      contract.client.toLowerCase().includes(termoBusca.toLowerCase())
    )
    .sort((a, b) => {
    ...
    <Layout active="Contato">
      <main className="min-h-[900px] bg-white font-sans">
        <section className="bg-soft-panel py-16 text-center border-b border-gray-100">
          <h1 className="text-3xl md:text-4xl font-bold text-[#364153]">Histórico de Contratos</h1>
          <p className="text-gray-500 mt-2 text-sm">Gerencie os serviços prestados aos seus clientes.</p>
        </section>
        ...
        <section className="grid gap-8 px-4 py-8 md:grid-cols-[240px_1fr] md:px-11 max-w-7xl mx-auto">
          ...
          <aside className="rounded-xl border border-gray-200 bg-gray-50 p-5 text-sm text-gray-700 h-fit">
            <h3 className="mb-4 text-sm font-bold text-[#364153] uppercase tracking-wider">Filtros</h3>

            <div className="mb-6 space-y-2">
              {["Essa semana", "Esse mês", "Esse ano"].map((item) => (
                <div key={item} className="flex w-fit items-center gap-2 rounded-md bg-white border border-gray-200 px-2.5 py-1.5 text-xs font-medium text-gray-600 shadow-sm">
                  {item} <span className="cursor-pointer hover:text-red-500">×</span>
                </div>
              ))}
            </div>
            ...
            <div className="mb-6">
              <div className="mb-2 flex justify-between font-medium">
                <span>Preço</span><span className="text-[#00A63E]">R$0 - R$500</span>
              </div>
              <input type="range" className="w-full accent-[#00A63E]" />
            </div>

            <div className="mb-6">
              <h3 className="mb-3 text-sm font-bold text-[#364153]">Serviço</h3>
            ...
            <div>
              <h3 className="mb-3 text-sm font-bold text-[#364153]">Cidade do Cliente</h3>
          ...
          <div>
            ...
            {/* CONTEÚDO CONDICIONAL (LOADING / SELEÇÃO DINÂMICA) */}
            {isLoading ? (
        ...
        <div className="px-4 pb-12 md:px-11 max-w-7xl mx-auto">
          <Link to="/prestador/home" className="inline-flex h-11 items-center justify-center rounded-lg bg-[#00A63E] px-6 text-sm font-bold text-white transition hover:bg-[#008F35] shadow-sm">
